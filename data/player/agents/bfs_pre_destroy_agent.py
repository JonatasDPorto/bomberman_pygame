...
class BFSPreDestroyAgent(PlayerAgent):
    ...
    def remove_destroyables_around(self, x, y, destroyable_matrix):
        if x > 0:
        ...
        if y < GRID_SIZE - 1:
            destroyable_matrix[x][y + 1] = 0
        return destroyable_matrix


    def pre_destroy_destroyables(self, destroyable_matrix, bombs_matrix):
        for x in range(GRID_SIZE):
            for y in range(GRID_SIZE):
                if self.there_is_a_bomb_in_my_position(x, y, bombs_matrix):
                    destroyable_matrix = self.remove_destroyables_around(x, y, destroyable_matrix)
    ...
    def act(self, playerStateInformation: PlayerStateInformation, gameStateInformation: GameStateInformation) -> PlayerAction:
        ...
        destroyable_matrix = self.pre_destroy_destroyables(destroyable_matrix, gameStateInformation.bombs_matrix)
        ...
        next_move = self.start_bfs(destroyable_matrix, gameStateInformation.walls_matrix, playerStateInformation.x, playerStateInformation.y)
